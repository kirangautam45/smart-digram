...
const ArchitectureDiagramView = () => {
  ...
  const onNodeDragStart = useCallback(
    (event, node) => {
      ...
      if (node.type === "service") {
        ...
        setDragStartState(dragState);

        // Remove extent constraint to allow dragging between groups
        setNodes((nds) =>
          nds.map((n) => {
            if (n.id === node.id) {
              return {
        ...
        // For group nodes, just mark as dragging
        setNodes((nds) =>
          nds.map((n) =>
            n.id === node.id
              ? { ...n, dragging: true }
  ...
  const onNodeDrag = useCallback(
    (event, node) => {
      // Update node position during drag for all node types
      setNodes((nds) =>
        nds.map((n) =>
          n.id === node.id
  ...
  const onNodeDragStop = useCallback(
    (event, node) => {
      ...
      console.log('📦 Current dragStartState:', dragStartState);
      ...
      // Retrieve original parent from node data or dragStartState
